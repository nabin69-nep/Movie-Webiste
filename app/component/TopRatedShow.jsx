...
export default function Trending() {
  const {  setTvId} = useContext(IdContext);
    ...
    const getResponse = async () => {
      try {
        const res = await fetch(
          `https://api.themoviedb.org/3/tv/top_rated?api_key=233d579ffe391c65ea271864eb408536&page=${page}`
        );
        if (!res.ok) {
...
<Carousel>
  <CarouselContent>
          {data.map((movie) => (
    <CarouselItem key={movie.id} className="md:basis-1/2 py-5 lg:basis-1/3">
       <div
              key={movie.id}
            ...
            >
              ...
              />
              <div className="p-4">
                <Link  onClick={()=>setTvId(movie.id)} href={`/section/tv/${
...
<Carousel>
  <CarouselContent>  
    {data.map((movie, index) => {
       return  <CarouselItem key={movie.id} className="py-5 text-center basis-1/3">
  <button
          key={index}
