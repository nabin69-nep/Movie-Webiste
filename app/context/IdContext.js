  ...
  const [searchPeople, setSearchPeople] = useState([]);
    const getResponse = async () => {
      try {
        ...
        const peopleRes = await fetch(`https://api.themoviedb.org/3/search/person?api_key=233d579ffe391c65ea271864eb408536&query=${word}`);
        const tvRes = await fetch(`https://api.themoviedb.org/3/search/tv?api_key=233d579ffe391c65ea271864eb408536&query=${word}`);
        if (!movieRes.ok) {
